...
const CartProductList = ({ cartItems: propCartItems }) => {
  ...
  const toggleSelectAll = () => {
    ...
    setLocalCartItems(
      localCartItems.map((item) => ({ ...item, selected: !allSelected }))
    );
  };

  const handleQuantityChange = (id, amount) => {
    const currentQuantity =
      localCartItems.find((item) => item.id === id)?.quantity || 0;
  ...
  const subtotal = localCartItems.reduce(
    (total, item) =>
      item.selected
        ? total + parseFloat(item.product.current_price || 0) * item.quantity
        : total,
    0
  );
    ...
    <section className="mx-auto px-8 md:px-10 py-10">
      <div className="flex flex-row justify-between flex-wrap items-start gap-4 w-full">
        <div className="cart-container  ">
          ...
          <div className="cart-items">
            {localCartItems.map((item) => (
              <div
                key={item.id}
                className="cart-item border mt-4 flex flex-wrap justify-between"
              >
                <div className="flex gap-1">
                  ...
                  <div className="flex flex-wrap">
                    ...
                    <div className="mt-3  flex-col">
                      <span className=" text-base font-[400px]">
                        {item.product.name}
                      </span>
                      <br />
                      <span>
                ...
                <div className="flex mt-3 items-end gap-2 flex-col flex-wrap justify-between p-2">
                  <div className="items-center flex">
                    <span>
                      ₦
                      {(
                        (parseFloat(item.product.current_price || 0) || 0) *
                        item.quantity
                      ).toLocaleString()}
                    </span>
                    <button
        ...
        <div className="cart-summary border mt-11 p-3 grow rounded">
          ...
          <div className="p-3">
            <Link to="/delivery"> {/* Changed the Link to /delivery */}
